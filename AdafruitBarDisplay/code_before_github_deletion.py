...
# WiFi credentials
WIFI_SSID = "MaggieNet"
WIFI_PASSWORD = "MoreTreatsPlease!"
...
sonos_is_on = True  # Track if Sonos is playing music
sonos_off_start_time = 0  # When Sonos went off
DISPLAY_POWER_OFF_TIMEOUT = 300  # 5 minutes (300 seconds)
display_powered_off = False  # Track if display is currently off
...
def turn_off_display():
    """Turn off the display by setting it to black"""
    global display_powered_off
    
    try:
        # Create a black bitmap
        bitmap = displayio.Bitmap(320, 960, 1)
        ...
        tprint("🔌 Display turned OFF (black screen)")
        
    except Exception as e:
        tprint(f"❌ Error turning off display: {e}")

def turn_on_display():
    """Turn on the display (will show next image)"""
    global display_powered_off
    
    display_powered_off = False
    tprint("🔌 Display turned ON (ready for images)")

def fetch_metadata():
    """Fetch current song metadata"""
    global current_metadata, sonos_is_on, sonos_off_start_time
    ...
    if response:
        ...
        try:
            ...
            # Check sonos_is_on status
            previous_sonos_status = sonos_is_on
            sonos_is_on = data.get("sonos_is_on", True)  # Default to True if not present
            
            # Handle power management
            if sonos_is_on:
                if not previous_sonos_status:
                    tprint("🎵 Sonos turned ON - turning on display")
                    turn_on_display()
                    sonos_off_start_time = 0
            else:
                if previous_sonos_status:
                    tprint("🔇 Sonos turned OFF - starting power-off timer")
                    sonos_off_start_time = time.monotonic()
                else:
                    # Check if we should turn off display
                    if sonos_off_start_time > 0:
                        time_since_off = time.monotonic() - sonos_off_start_time
                        if time_since_off >= DISPLAY_POWER_OFF_TIMEOUT and not display_powered_off:
                            tprint(f"🔌 Sonos off for {time_since_off:.0f}s - turning off display")
                            turn_off_display()
                        elif time_since_off < DISPLAY_POWER_OFF_TIMEOUT:
                            remaining = DISPLAY_POWER_OFF_TIMEOUT - time_since_off
                            tprint(f"⏰ Sonos off for {time_since_off:.0f}s - display off in {remaining:.0f}s")
            ...
            status_text = "ON" if sonos_is_on else "OFF"
            tprint(f"✅ Metadata: {current_metadata['title']} - {current_metadata['artist']} (Sonos: {status_text})")
            return True
        except Exception as e:
...
def check_if_image_needed():
    ...
    # Skip image updates if display is powered off
    if display_powered_off:
        tprint("🔌 Display is powered off - skipping image updates")
        return False, False
...
def download_and_display_image():
    ...
    # Skip if display is powered off
    if display_powered_off:
        tprint("🔌 Display is powered off - skipping image download")
        return False
...
tprint("✓ Power management: Display off after 5 minutes of no music")
