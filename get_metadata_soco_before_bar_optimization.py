...
def create_blank_screen(bmp_path):
    ...
    try:
        ...
        try:
            ...
            # Rotate the blank composite 90 degrees clockwise for portrait display
            blank_composite_rotated = blank_composite.rotate(-90, expand=True)  # -90 = clockwise
            
            # Save directly as BMP
            temp_bar_bmp = BMP_BAR_PATH + ".temp"
            blank_composite_rotated_indexed = blank_composite_rotated.quantize(colors=256, method=2, dither=1)
            blank_composite_rotated_indexed.save(temp_bar_bmp, format="BMP", compression=0)
            
            # Move to final location
            shutil.move(temp_bar_bmp, BMP_BAR_PATH)
            
            print("✓ Blank rotated bar composite successfully created (320x960)")
        except Exception as bar_error:
            print(f"✗ Blank rotated bar composite error: {bar_error}")
...
def is_music_playing(soco_track, control_track):
    ...
    # For debugging
    print(f"Music detection - Title: '{title}', Position: {position}, Duration: {duration}, State: {player_state}")
    print(f"Music detection - Has title: {has_title}, Has playback: {has_playback}, Is playing: {is_playing}, Position advancing: {position_advancing}")
...
def create_bar_artwork(source_bmp_path, bar_bmp_path, title="", artist="", album=""):
    ...
    try:
        ...
        # Create base 960x320 canvas
        composite_width = 960
        composite_height = 320
        artwork_size = 320  # Square artwork on the left
        ...
        try:
            ...
            # Text area: starts after artwork (x=320) to end (x=960)
            text_start_x = artwork_size + 30  # 30px margin
            text_width = composite_width - artwork_size - 60  # Available width
        ...
        # Rotate the composite 90 degrees clockwise for portrait display
        ...
        try:
            ...
            # Use fewer colors and simpler quantization for faster ESP32 loading
            composite_rotated_indexed = composite_rotated.quantize(
                colors=64,    # Reduced from 256 - ESP32 processes fewer colors faster
                method=0,     # Simple quantization instead of median cut (method=2)
                dither=0      # No dithering - simpler for ESP32 to process
            )
            
            # Save with ESP32-optimized BMP settings  
...
def main():
    ...
    print("Starting Sonos metadata monitor...")
    print("Press Ctrl+C to exit")
    print(f"Will show blank screen after {MUSIC_TIMEOUT_SECONDS} seconds of no music")
    print("OPTIMIZED: Only process artwork when song changes, check every 1 second")
    print("FAST RESPONSE: 1-second polling for immediate song change detection")
    ...
    while True:
        try:
            ...
            try:
                ...
                if cpu_percent > 80 or memory.percent > 85:
                    print(f"⚠️ High system load: CPU {cpu_percent:.1f}%, Memory {memory.percent:.1f}%")
                    print("Sleeping longer to prevent overload...")
            ...
            if time_since_music > MUSIC_TIMEOUT_SECONDS and not blank_screen_shown:
                ...
                if create_blank_screen(BMP_PATH):
                    ...
                    print("✓ Blank screen displayed with empty metadata")
            ...
            for speaker in soco_devices:
                ...
                try:
                    ...
                    if is_music_playing(soco_track, control_track):
                        ...
                        # Debug: Print all available track info
                        print("\nDEBUG - Available track info:")
                        print("SoCo track info:", json.dumps(soco_track, indent=2))
                        print("Control API track info:", json.dumps(control_track, indent=2))
                        ...
                        # Music is not playing, just log the detection info
                        print("Music detection - Title: 'None', Position: 0:00:00, Duration: 0:00:00, State:")
                        print("Music detection - Has title: None, Has playback: False, Is playing: False")
                        print("No music playing, skipping metadata processing")
            ...
            if not music_found:
                time_since_music = current_time - last_music_detected
                print(f"No music detected. Time since last music: {time_since_music:.0f} seconds")
                if time_since_music > MUSIC_TIMEOUT_SECONDS and not blank_screen_shown:
                    print(f"Showing blank screen after {time_since_music:.0f} seconds of no music")
                    if create_blank_screen(BMP_PATH):
                        copy_to_qualia(BMP_PATH)
                        # Clear global metadata and save empty JSON for Qualia displays
                        current_song_title = ""
        ...
        print(f"💤 Sleeping 1 second before next check...")
